...
(function(){
  ...
  function renderHome() {
    ...
    if (presets.length > 1) {
      ...
      html += '<div class="preset-bar-scroll" id="preset-bar-scroll">';
      presets.forEach(function(p) {
        var isActive = preset.id === p.id;
        html += '<button class="preset-tab' + (isActive ? ' active' : '') + '" data-preset-id="' + escHtml(p.id) + '">' + escHtml(p.name) + '</button>';
      });
  ...
  /* ========== プリセットバー 長押しドラッグ並び替え ========== */
  function _initPresetBarDrag() {
    var scroll = document.getElementById('preset-bar-scroll');
    if (!scroll) return;
    ...
    function getPresetTabs() {
      return Array.from(scroll.querySelectorAll('.preset-tab[data-preset-id]'));
    }
    ...
    function onPointerDown(e, isMouse) {
      var tab = e.target.closest('.preset-tab[data-preset-id]');
      if (!tab) return;
      if (isMouse && e.button !== 0) return;
      ...
      startX = pos.x;
      startY = pos.y;
      tappedId = tab.getAttribute('data-preset-id');
      if (isMouse) isMouseDown = true;
      ...
      longPressTimer = setTimeout(function() {
        ...
        placeholder = document.createElement('div');
        placeholder.className = 'preset-tab-placeholder';
        placeholder.style.width = rect.width + 'px';
        placeholder.style.height = rect.height + 'px';
        placeholder.style.flexShrink = '0';
        dragItem.parentNode.insertBefore(placeholder, dragItem);

        dragItem.classList.add('preset-tab-dragging');
        dragItem.style.position = 'fixed';
        dragItem.style.left = (rect.left - _txOff.x) + 'px';
        dragItem.style.top = (rect.top - _txOff.y) + 'px';
        dragItem.style.width = rect.width + 'px';
    ...
    function onPointerMove(e) {
      ...
      var tabs = getPresetTabs().filter(function(el) { return el !== dragItem; });
      var addBtn = scroll.querySelector('.preset-tab-add');
      var inserted = false;
      for (var i = 0; i < tabs.length; i++) {
        var r = tabs[i].getBoundingClientRect();
        if (pos.x < r.left + r.width / 2) {
          scroll.insertBefore(placeholder, tabs[i]);
          inserted = true;
          break;
        }
      }
      if (!inserted) {
        if (addBtn) scroll.insertBefore(placeholder, addBtn);
      }
    }
    ...
    function onPointerEnd() {
      ...
      if (!isDragging) {
        if (tappedId) {
          switchPreset(tappedId);
          tappedId = null;
        }
        return;
      }

      dragItem.classList.remove('preset-tab-dragging');
      dragItem.style.position = '';
      dragItem.style.left = '';
      dragItem.style.top = '';
      dragItem.style.width = '';
      ...
      var newOrder = [];
      getPresetTabs().forEach(function(tab) {
        var pid = tab.getAttribute('data-preset-id');
        if (pid) newOrder.push(pid);
      });
    ...
    // Touch
    scroll.addEventListener('touchstart', function(e) { onPointerDown(e, false); }, { passive: true });
    scroll.addEventListener('touchmove', function(e) { onPointerMove(e); }, { passive: false });
    scroll.addEventListener('touchend', function() { onPointerEnd(); }, { passive: true });
    scroll.addEventListener('touchcancel', function() { onPointerCancel(); }, { passive: true });

    // Mouse
    scroll.addEventListener('mousedown', function(e) { onPointerDown(e, true); });
    document.addEventListener('mousemove', function(e) {
      if (!isMouseDown && !isDragging) return;
      onPointerMove(e);
    });
